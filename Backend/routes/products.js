...
router.get('/products/:id', async (req, res) => {
  ...
  if (!product) {
    return res.status(401).json({ message: "Produkten hittades inte" })
      ...
      res.status(404).json({ message: "Ett fel uppstod vid hämtning av produkten" });
...
router.post('/products/delete/:id', async (req, res) => {
  ...
  if (product) {
    ...
    res.status(401).json({ message: "Produkten hittades inte" });
  }
} catch (error) {
  console.error("Ett fel uppstod vid borttagning av produkten:", error);
  res.sendStatus(500).json({ message: "Ett fel uppstod vid borttagning av produkten"});
}
...
router.put('/products/update/:id', async(req, res) => {
  try {
    ...
    res.sendStatus(500).json({ message: "Ett fel uppstod vid uppdateringen av produkten"});
  }
});
